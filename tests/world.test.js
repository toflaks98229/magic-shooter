...
test('beginFloor는 층에 매인 것만 비우고 판에 걸친 것은 남긴다', () => {
    ...
    for (const key of worldModule.FLOOR_SCOPED_COLLECTIONS) {
        // 옛 층의 것이 남지 않았는지를 봅니다. 비어 있는지가 아니라,
        // 지난 층에서 넣어 둔 표시가 사라졌는지가 이 검사의 뜻입니다.
        // vaults 는 비운 뒤 새 층의 볼트로 곧바로 채워지므로 길이가 0 이 아닙니다.
        const leftover = world[key].some(item => item?.marker === key);
        assert.equal(leftover, false, `${key}에 옛 층의 것이 남았습니다`);
    }
