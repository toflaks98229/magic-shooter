...
function renderWorld(width, height) {
    ...
    for (let i = 0; i < width; i++) {
        ...
        const wallTopY = halfHeight - wallHeight / 2;
        const wallBottomY = halfHeight + wallHeight / 2;
        const y0 = Math.max(0, wallTopY | 0);
        const y1 = Math.min(height, wallBottomY | 0);

        // --- 새로운 벽 텍스처 선택 로직 ---
        let wallTextureInfo;
        // 맵 데이터가 4인 경우는 출구로 취급합니다.
        if (ray.texture === 4) {
            wallTextureInfo = S.textures.exit || placeholderTex;
        } else { // 그 외 모든 벽(이제 1번으로 통일됨)
            // 벽의 월드 좌표(mapX, mapY)를 기반으로 시드 값을 만들어 매 프레임 동일한 텍스처가 선택되도록 합니다.
            // 이렇게 하면 벽 텍스처가 프레임마다 무작위로 바뀌는 것을 방지할 수 있습니다.
            const wallHash = (ray.mapX * 19 + ray.mapY * 73); // 간단한 해시 함수
        ...
        // 밝기 계산 (거리에 따른 감쇠 + 벽면에 따른 음영 + 동적 광원)
        const brightness = 1 - Math.min(correctedDist / (C.TILE_SIZE * 12), 1);
        const shade = ray.side === 1 ? 0.7 : 1.0; // Y축 벽을 더 어둡게
        ...
        // 바닥 및 천장 렌더링
        let floorXWall, floorYWall; // 광선이 벽에 닿은 정확한 월드 좌표
        ...
        for (let y = y1; y < height; y++) {
            ...
            const tileHash = (mapTileX * 23 + mapTileY * 83); // 벽과 다른 해시 함수 사용
            ...
            // 천장 픽셀을 버퍼에 씁니다 (화면 위아래 대칭).
            if (ceilingTexData) {
                const ceilY = height - 1 - y;
                const texIndex = (floorTexY * floorTexSize + floorTexX) << 2; // 천장은 바닥과 같은 텍스처 좌표를 사용
...
function renderEntities(targetCtx, width, height) {
    ...
    // 렌더링할 모든 엔티티를 하나의 배열로 합치고, 플레이어로부터의 거리를 계산합니다.
    const entitiesToRender = [...S.enemies, ...S.items, ...S.projectiles, ...S.particles]
        .map(e => {
...
function prepareSprites(sheetKey, sheetImage, atlas) {
    ...
    for (const key in atlas.sprites) {
        ...
        if (themeMatch) {
            ...
            // 테마 시스템에 해당하지 않는 나머지 스프라이트(e.g. exit)는 ImageData로 저장
             S.textures[key] = {
                data: tempCtx.getImageData(0, 0, coords.w, coords.h).data,
                w: coords.w,
                h: coords.h
