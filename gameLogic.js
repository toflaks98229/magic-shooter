...
export function interactWithWorld() {
    ...
    // 해당 타일이 출구(4)인지 확인합니다.
    if (S.map[tileY]?.[tileX] === 4) {
        // 여기에 '문 여는 소리' 같은 효과음을 추가할 수 있습니다.
        // window.playSound('EXIT_SOUND');
...
/**
 * 플레이어의 이동 입력을 받아 실제 위치를 업데이트하고 벽 충돌을 처리합니다.
...
function handlePlayerMovement(dtFactor) {
    ...
    if (S.isMoving) {
        ...
        // 벽 충돌 처리 (X축, Y축을 분리하여 벽에 부딪혔을 때 미끄러지도록 함)
        if (S.map[Math.floor(S.player.y / C.TILE_SIZE)]?.[Math.floor(nextX / C.TILE_SIZE)] === 0) {
            S.player.x = nextX;
        }
        if (S.map[Math.floor(nextY / C.TILE_SIZE)]?.[Math.floor(S.player.x / C.TILE_SIZE)] === 0) {
            S.player.y = nextY;
        }
    }
}
