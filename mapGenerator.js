 ...
 * @returns {{map: number[][], playerStart: {x: number, y: number}}} - 생성된 맵 배열과 플레이어 시작 좌표를 담은 객체
 */
export function generateDungeon(width, height, maxRooms, minRoomSize, maxRoomSize) {
    // 1. 맵을 모두 벽(1)으로 초기화합니다.
 ...
 * @returns {{map: number[][], playerStart: {x: number, y: number}}} - 생성된 맵과 플레이어 시작 좌표
 */
function createFallbackDungeon(width, height) {
    ...
    return { map: fallbackMap, playerStart };
}
